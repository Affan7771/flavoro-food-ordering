...
const Cart = () => {
    ...
    return (
        <>
            <div className={`fixed right-0 w-full top-0 lg:w-[20vw] h-full p-5 bg-white mb-3 transition-all duration-500 z-50 ${activeCart ? "translate-x-0" : "translate-x-full"}`}>
            ...
            <FaShoppingCart 
                ...
                className={`rounded-full bg-white shadow-md text-5xl p-3 fixed bottom-4 right-4 cursor-pointer ${totalQty > 0 && "transition-all animate-bounce delay-500"}`} 
            />
        </>
    )
}
